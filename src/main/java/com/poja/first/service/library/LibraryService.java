...
public class LibraryService {
  ...
  public List<LibraryDTO> getAllLibrary() {
    return libraryRepository.findAllWithBooks().stream().map(libraryMapper::toDTO).toList();
  }
  ...
  public LibraryDTO getLibraryById(String libraryId) {
    return libraryRepository
        ...
        .orElseThrow(
            () -> new NotFoundException(String.format("Library with id %s not found", libraryId)));
  }

  public List<LibraryDTO> searchLibraryByName(String keyword) {
    return libraryRepository.findByNameContainingIgnoreCase(keyword).stream()
        .map(libraryMapper::toDTO)
        .toList();
  }
